    ...
    for char, x in aoc_string.chars(row) do
      if char:match("%d") then
        ...
        for _, point in ipairs(Point.new(x, y):principals()) do
          if point.y > 0 and
             point.y <= #self.rows and
             self.rows[point.y]:sub(point.x, point.x):match("[^%.%d]")
          then
            has_symbol = true
            break
          end
    ...
    for char, x in aoc_string.chars(row) do
      if char == "*" then
        ...
        for _, point in ipairs(Point.new(x, y):principals()) do
          ...
          then
            ...
            for i = point.x, 1, -1 do
              if not self.rows[point.y]:sub(i, i):match("%d") then break end
              number_start = i
            end
        ...
        if aoc_table.count(numbers) == 2 then
          table.insert(
            ratios,
            aoc_table.product(numbers, function(_, key) return key end)
          )
        end
      end
    end
